...
fn planner_score(
    ...
    .unwrap();
    AdvisoryPlannerScore::new(objective)
}
...
fn test_advisory_score_cannot_bypass_budget_exhausted() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-1", 2_500))
        .unwrap();
...
fn test_advisory_score_cannot_bypass_circuit_breaker() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-3", 3_000))
        .unwrap();
    assert!(receipt.is_none());
...
fn test_advisory_score_cannot_bypass_freshness_check() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-1", 1_500))
        .unwrap();
    assert!(receipt.is_none());
...
fn test_advisory_score_cannot_bypass_work_completed() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-1", 2_000))
        .unwrap();
    assert!(receipt.is_none());
...
fn test_advisory_score_cannot_authorize_spawn() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-1", 2_100))
        .unwrap();
    assert!(receipt.is_none());
...
fn test_advisory_score_no_gate_mutation() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-1", 1_200))
        .unwrap();
    assert!(receipt.is_none());
...
fn test_receipt_emitted_at_tier3_threshold() {
    ...
    let receipt = controller
        .record_advisory_objective(best_possible_score("work-1", 4_000))
        .unwrap()
...
fn test_max_objectives_limit() {
    ...
    for index in 0..MAX_TRACKED_OBJECTIVES {
        ...
        let result = controller.record_advisory_objective(score).unwrap();
        assert!(result.is_none());
    }

    let overflow = best_possible_score(&format!("work-{MAX_TRACKED_OBJECTIVES}"), 9_999);
    let err = controller.record_advisory_objective(overflow).unwrap_err();
...
fn test_advisory_score_lookup_returns_latest_score() {
    ...
    controller.record_advisory_objective(first).unwrap();

    let second = planner_score("work-1", EfeComponents::new(0.2, 0.2, 0.2).unwrap(), 1_200);
    controller
        .record_advisory_objective(second.clone())
        .unwrap();

    let latest = controller.advisory_score_for("work-1").unwrap();
...
fn test_advisory_record_requires_known_work_id() {
    ...
    let err = controller.record_advisory_objective(unknown).unwrap_err();
