...
- [INV-LED-016] Fail-closed on data loss: if `ledger_events_legacy_frozen` has rows but `events` is empty after a prior migration, the migration fails with `MigrationPartialState` (canonical chain missing)
