...
//!    behavior
//! 4. **Audit events**: Denied reads emit [`ToolDecided`] events for logging
//!
//! # Firewall Modes
//!
...
/// Event emitted when a tool decision is made.
///
...
impl ToolDecided {
    ...
    #[must_use]
    pub fn deny_allowlist(path: impl Into<String>) -> Self {
        Self {
...
pub enum ValidationResult {
    ...
    ///
    /// Contains the [`ToolDecided`] event for audit logging.
    Denied {
...
impl ValidationResult {
    ...
    #[must_use]
    pub const fn event(&self) -> Option<&ToolDecided> {
        match self {
            Self::Allowed => None,
...
impl<'a> DefaultContextFirewall<'a> {
    ...
    fn handle_denial(&self, path: &str) -> Result<ValidationResult, ContextFirewallError> {
        let event = ToolDecided::deny_allowlist(path);
    ...
    fn handle_hash_mismatch(&self, path: &str) -> Result<ValidationResult, ContextFirewallError> {
        let event =
            ToolDecided::deny_with_reason(CTX_ALLOWLIST_RULE_ID, path, "Content hash mismatch");
...
pub mod tests {
    ...
    #[test]
    fn test_tool_decided_deny_allowlist() {
        let event = ToolDecided::deny_allowlist("/etc/passwd");
    ...
    #[test]
    fn test_tool_decided_deny_with_reason() {
        let event =
            ToolDecided::deny_with_reason("CUSTOM-RULE-001", "/some/path", "Custom denial reason");
    ...
    #[test]
    fn test_tool_decided_serde_roundtrip() {
        let event = ToolDecided::deny_allowlist("/test/path");
        let json = serde_json::to_string(&event).unwrap();
        let recovered: ToolDecided = serde_json::from_str(&json).unwrap();
        assert_eq!(event, recovered);
    }

    #[test]
    fn test_tool_decided_deny_unknown_fields() {
        let json = r#"{
            "decision": "DENY",
        ...
        let result: Result<ToolDecided, _> = serde_json::from_str(json);
        assert!(result.is_err());
    ...
    fn test_validation_result_denied() {
        let event = ToolDecided::deny_allowlist("/test");
