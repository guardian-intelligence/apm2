...
impl SqliteTimeoutLedgerReader {
    ...
    /// Event types observed by the gate timeout kernel.
    const OBSERVED_EVENT_TYPES: &'static [&'static str] = &[
        "gate_lease_issued",
        "gate.timed_out",
        "gate.receipt",
        "GateReceipt",
    ...
    /// Polls the ledger using the shared freeze-aware poller and maps
    /// `SignedLedgerEvent` results to `TimeoutObservedEvent`.
    ///
    /// TCK-00675: Replaces the 8 bespoke `query_*` functions and manual
    /// merge/sort/truncate with a single call to
    /// [`crate::ledger_poll::poll_events_blocking`].
    fn poll(
        &self,
        cursor: &CompositeCursor,
        limit: usize,
        ...
        let raw_events = crate::ledger_poll::poll_events_blocking(
            &guard,
        ...
        // Map SignedLedgerEvent -> TimeoutObservedEvent using the existing
        // domain-specific parse helpers.
        ...
        for event in raw_events {
            let kind = match event.event_type.as_str() {
                ...
                other => {
                    // Should not happen since the poller filters by
                    // OBSERVED_EVENT_TYPES, but fail-closed.
                    return Err(format!("unexpected event type from shared poller: {other}"));
                },
            };
