...
impl LedgerTailer {
    ...
    // NOTE: `is_canonical_active` and `poll_canonical_events` have been
    // removed — canonical detection and dual-table polling are now handled
    // by the shared `crate::ledger_poll` module (TCK-00675).
    ...
    /// Gets the next batch of unprocessed events of a given type.
    ///
    /// Delegates to [`crate::ledger_poll::poll_events_blocking`] for
    /// freeze-aware dual-table (legacy + canonical) polling with consistent
    /// cursor semantics.
    ...
    pub fn poll_events(
        ...
        let cursor_ts = i64::try_from(self.last_processed_ns).map_err(|_| {
            ProjectionWorkerError::DatabaseError("cursor timestamp exceeds i64 range".to_string())
        })?;
    ...
    /// Async wrapper for `poll_events` via the shared
    /// [`crate::ledger_poll::poll_events_async`] poller.
    ///
    /// Delegates freeze-aware dual-table polling to the shared module,
    /// offloading `SQLite` I/O to `tokio::task::spawn_blocking`.
    #[allow(clippy::cast_sign_loss, clippy::cast_possible_wrap)]
