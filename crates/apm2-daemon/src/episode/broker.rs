...
pub struct ToolBroker<L: ManifestLoader = super::capability::StubManifestLoader> {
    ...
    /// At Tier2+, any missing component causes a deny-default decision.
    path_ratchet: super::path_ratchet::PathRatchet,
}
...
impl<L: ManifestLoader + Send + Sync> ToolBroker<L> {
    ...
    pub async fn request_with_response(
        ...
        // Step 2b: Epoch seal verification for Tier2+ admission (TCK-00365).
        //
        ...
        // denied (fail-closed).
        //
        // The request-carried epoch seal is consumed here when present.
        ...
        // causes a deny-default decision. This prevents bypass via missing manifests
        // or unconfigured enforcement components.
        {
            ...
            let ratchet_input = PathRatchetInput {
                ...
                context_firewall_status: context_status,
                capsule_admission_status: capsule_status,
            };
            ...
            match outcome {
                PathRatchetOutcome::Denied { error } => {
                    ...
                    respond!(ToolDecision::Deny {
                        request_id: request.request_id.clone(),
                        reason: DenyReason::PolicyDenied {
                            rule_id: "PATH_RATCHET".to_string(),
                            reason: error.to_string(),
                        },
                        rule_id: Some("PATH_RATCHET".to_string()),
                        policy_hash: self.policy.policy_hash(),
                    });
                },
                PathRatchetOutcome::AllowedWithWarnings { warnings } => {
                    for w in &warnings {
                        warn!(
                            risk_tier = request.risk_tier.tier(),
                            warning = %w,
                            "path ratchet warning (non-mandatory tier)"
                        );
                    }
                },
...
"#;
    ...
    /// Helper: creates a broker with accept-all signature verifier and a
    /// Read capability on `/workspace`.
    async fn make_epoch_seal_test_broker() -> ToolBroker<StubManifestLoader> {
        let broker: ToolBroker<StubManifestLoader> = ToolBroker::new(test_config_without_policy());
        let manifest = make_manifest(vec![make_read_capability(
            "cap-read",
            vec![PathBuf::from("/workspace")],
    ...
    async fn broker_epoch_seal_denies_tier2_without_seal() {
        let broker = make_epoch_seal_test_broker().await;

        let request = make_request_at_tier("req-seal-1", RiskTier::Tier2, None);
        let decision = broker
            .request(&request, timestamp_ns(0), None)
            .await
            .unwrap();
    ...
    async fn broker_epoch_seal_allows_tier0_without_seal() {
        ...
        let request = make_request_at_tier("req-seal-2", RiskTier::Tier0, None);
        let decision = broker
            .request(&request, timestamp_ns(0), None)
            .await
            .unwrap();
    ...
    async fn broker_epoch_seal_allows_tier1_without_seal() {
        ...
        let request = make_request_at_tier("req-seal-3", RiskTier::Tier1, None);
        let decision = broker
            .request(&request, timestamp_ns(0), None)
            .await
            .unwrap();
    ...
    async fn broker_epoch_seal_valid_accepted() {
        ...
        let request = make_request_at_tier("req-seal-4", RiskTier::Tier2, Some(seal));
        let decision = broker
            .request(&request, timestamp_ns(0), None)
            .await
            .unwrap();
    ...
    async fn broker_epoch_seal_rollback_denied() {
        ...
        let request5 = make_request_at_tier("req-seal-5a", RiskTier::Tier2, Some(seal5));
        let decision5 = broker
        ...
        let request3 = make_request_at_tier("req-seal-5b", RiskTier::Tier2, Some(seal3));
        let decision3 = broker
    ...
    async fn broker_epoch_seal_missing_seal_denied_at_tier3() {
        ...
        let request = make_request_at_tier("req-seal-6", RiskTier::Tier3, None);
        let decision = broker
            .request(&request, timestamp_ns(0), None)
            .await
            .unwrap();
        ...
        assert!(
            decision.is_denied(),
            "Tier3 without epoch seal must be denied"
        );
        if let ToolDecision::Deny { rule_id, .. } = &decision {
            assert_eq!(rule_id.as_deref(), Some("EPOCH_SEAL"));
        }
    ...
    async fn broker_epoch_seal_rejected_by_signature_verifier() {
        ...
        let seal = make_epoch_seal(1);
        let request = make_request_at_tier("req-seal-7", RiskTier::Tier2, Some(seal));
        let decision = broker
            .request(&request, timestamp_ns(0), None)
            .await
            .unwrap();
    ...
    // components are missing. They serve as regression tests to prevent
    // bypass reintroduction.
    // =========================================================================

    /// Helper: make a request with a specific risk tier.
    fn make_tiered_request(
        id: &str,
        tool_class: ToolClass,
        path: Option<&str>,
        risk_tier: RiskTier,
    ...
    /// context pack.
    #[tokio::test]
    async fn test_path_ratchet_tier2_no_context_manifest_denied() {
        let broker: ToolBroker<StubManifestLoader> = ToolBroker::new(test_config_without_policy());
        ...
        // NO context manifest loaded - this is the bypass we're testing against

        // Tier2 write request should be DENIED by path ratchet
        let request = make_tiered_request(
            "req-ratchet-1",
            ToolClass::Write,
            Some("/workspace/file.rs"),
        ...
        assert!(
            decision.is_denied(),
            "Tier2 without context manifest must be denied by path ratchet, got: {decision:?}"
        );
        if let ToolDecision::Deny {
            reason, rule_id, ..
        } = &decision
        {
            ...
            assert!(
                reason_str.contains("context firewall"),
                "denial reason must mention context firewall: {reason_str}"
            );
        }
    }
    ...
    async fn test_path_ratchet_tier3_no_context_manifest_terminates() {
        ...
        // Tier3 read request without context manifest
        let request = make_tiered_request(
            "req-ratchet-2",
            ToolClass::Read,
            Some("/workspace/file.rs"),
    ...
    async fn test_path_ratchet_tier0_no_context_manifest_allowed() {
        ...
        // NO context manifest - Tier0 should still be allowed
        let request = make_tiered_request(
            "req-ratchet-4",
            ToolClass::Write,
            Some("/workspace/file.rs"),
        ...
        assert!(
            decision.is_allowed(),
            "Tier0 without context manifest should be allowed, got: {decision:?}"
        );
    }
    ...
    async fn test_path_ratchet_tier1_no_context_manifest_allowed() {
        ...
        assert!(
            decision.is_allowed(),
            "Tier1 without context manifest should be allowed, got: {decision:?}"
        );
    }
    ...
    async fn test_path_ratchet_regression_no_bypass_at_mandatory_tiers() {
        for tier in [RiskTier::Tier2, RiskTier::Tier3, RiskTier::Tier4] {
            ...
            // No context manifest loaded
            let request = make_tiered_request(
                &format!("req-bypass-{}", tier.tier()),
                ToolClass::Write,
                Some("/workspace/file.rs"),
                tier,
            );
