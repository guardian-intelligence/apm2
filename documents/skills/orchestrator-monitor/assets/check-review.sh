...
PULSE_SECURITY="${HOME}/.apm2/review_pulse_security.json"
PULSE_QUALITY="${HOME}/.apm2/review_pulse_quality.json"
  ...
  if command -v jq >/dev/null 2>&1; then
    if [[ -n "$PR_FILTER" ]]; then
      jq -r --argjson pr "$PR_FILTER" '
        .reviewers
        | to_entries
        | map(select((.value.pr_url // "") | test("/pull/" + ($pr|tostring) + "$")))
        | if length == 0 then "  (no active entries for filter)"
          else .[] | "  \(.key): pid=\(.value.pid) model=\(.value.model // "n/a") backend=\(.value.backend // "codex") sha=\(.value.head_sha // "n/a") restarts=\(.value.restart_count // 0) log=\(.value.log_file // "n/a")"
          end
      ' "$STATE_FILE"
    else
      jq -r '
...
echo "--- Pulse Files ---"
for f in "$PULSE_SECURITY" "$PULSE_QUALITY"; do
